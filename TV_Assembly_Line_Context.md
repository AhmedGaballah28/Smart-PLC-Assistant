...
**Completed Stations: 5 of 7** ✅ (+ Warehouse)
...
│       ├── station6.py                  (✅ Quality Control - WORKING)
│       ├── station7.py                  (✅ Sorting & Output - WORKING)
│       └── warehouse.py                 (✅ Automated Warehouse - WORKING)
│
├── docs/
...
└── tests/
    ...
    ├── run_station6_test.py             (🔍 Station 6 runner)
    ├── run_station7_test.py             (🔀 Station 7 runner)
    ├── run_warehouse_test.py            (📦 Warehouse runner)
...
| 6       | Quality Control     | Vision Sensor, Stack Light          | 3s    | ✅ DONE    |
| 7       | Sorting & Output    | Pivot Arm Sorter, 2 Removers        | 2s    | ✅ DONE    |
...
### Station 6: Quality Control ✅ WORKING

**What it does:** Uses a Vision Sensor to inspect the product. If the Vision Sensor detects a proper Lid (value 5 = Green Product Lid), it marks QC_Result as PASS. If it detects only a chassis (value 3 = Blue Product Base) or anything else, it marks FAIL. 

**Components used:**
- 1× Belt Conveyor (Belt 4)
- 1× Stop Blade (Blade 3)
- 1× Vision Sensor
- Stack Light (Green/Red)

**State Machine:**
- ARRIVED → Inspect 3s → Read Vision Sensor → Set QC_Result → EXITING

**Runner:** 	ests/run_station6_test.py  
**Status: TESTED AND WORKING ✅**

---

### Station 7: Sorting & Output ✅ WORKING

**What it does:** Receives the product and the QC_Result from Station 6. A Pivot Arm Sorter directs the product: 
- PASS → straight ahead to good parts bin.
- FAIL → diverted right (45°) to reject bin.

**Components used:**
- 1× Main Belt (Belt 5)
- 1× Pivot Arm Sorter (Turn + Belt Fwd)
- 2× Removers (Good / Reject ends)
- Diffuse Sensors (Entry, Exit Good, Exit Reject)

**Runner:** 	ests/run_station7_test.py  
**Status: TESTED AND WORKING ✅**

---

### Warehouse Controller ✅ WORKING

**What it does:** Uses a 3-Rack (54 cells) Stacker Crane in Numerical Mode to pickup assembled products from a conveyor, store them in a rack cell, and later retrieve them or put them down.

**Features:**
- Real Modbus register integration (via pymodbus registers).
- Complete macro sequences for PICKUP, STORE, RETRIEVE, and PUTDOWN.
- Auto-store mode handling continuous feeding from a platform.
- Managed IO inputs and outputs mapping to exact crane behaviors (LIFT ON, MIDDLE, LEFT, RIGHT).

**Runner:** 	ests/run_warehouse_test.py  
**Status: TESTED AND WORKING ✅**
...
| Warehouse loading conveyor doesn't stop items | Polling cycle too slow to catch trigger immediately | **Fixed:** Instant stop executed immediately after sensor read inside the warehouse loop, averting overshoot. |
| Transfer arm drops items continuously | Pick and place arm initiating Z-axis movement instantly | **Fixed:** Inserted a deliberate `0.5s` settle delay when arm reaches the X coordinate. |
| Station 7 arm hits rejected products | Arm returning to straight position too hastily | **Fixed:** Injected `PRODUCT_CLEAR_TIME` (6.0s) delay explicitly on the REJECT branch to ensure product clearance. |
| Station 7 pushing prior to transfer clear | State machine lacking downstream hand-off visibility | **Fixed:** Added verification on Input 12; Station 7 waits for the Transfer P&P to lift the product before completing its cycle. |
...
| 6 - Quality Control        | ✅ DONE        | March 29, 2026 |
| 7 - Sorting                | ✅ DONE        | March 29, 2026 |

---

## 🎉 Latest Achievement

**Station 6, Station 7, Transfer Station, and the Warehouse Complete & Synchronized!** 🎉

- **Station 6 (QC):** Full vision sensor integration and PASS/FAIL assignment.
- **Station 7 (Sorting):** Pivot arm routing logic for good vs reject parts. Wait timings (`PRODUCT_CLEAR_TIME` = 6.0s) enforced for reject arm clearing. Hand-offs fully synchronized with Transfer station using Input 12.
- **Transfer Station:** 2-Axis Pick & Place moves items from the sorting line into the warehouse staging area. Includes a built-in `0.5s` settle delay before grabbing to guarantee suction security.
- **Warehouse:** Fully numerical 54-cell Stacker Crane with Store/Retrieve logic! Remapped entry sensor to Input 23, and fully integrated the loading conveyor instantly stopping upon sensor trigger. Transfer station patiently waits for the stacker to return home before placing a new pallet.
- **Line Testing:** `run_line.py` completely strings together Stations 1, 2, 3, 6, 7, Transfer, and Warehouse utilizing `threading.Event()` pipelines!

**Next:** Connect Everything together or Build Station 4/5.
